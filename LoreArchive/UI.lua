...
-- Slash Commands (Using a simpler, standard registration)
...
print("|cFF00FFFF[Lore Archive]|r UI Handler registered.")

-- Main initialization wrapped in a function for error catching
local function Initialize()
    ...
    -- Progress Counter
    MainFrame.ProgressText = MainFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    MainFrame.ProgressText:SetPoint("TOP", MainFrame.Title, "BOTTOM", 0, -2)
    MainFrame.ProgressText:SetTextColor(0.8, 0.8, 0.8)
    MainFrame.ProgressText:SetText("0 / 0 collected")
    ...
    local function ShowLoreFragment(fragment)
        ...
        if isHTML then
            ...
            clean = clean:gsub("&nbsp;", " ") -- SimpleHTML doesn't support nbsp
    ...
    function UI.UpdateList()
        ...
        for _, book in ipairs(db.books) do
            local titleMatch = not searchText or searchText == "" or
                (book.title and book.title:lower():find(searchText)
                )
...
-- Execute initialization and catch errors for BugSack/BugGrabber
local ok, err = pcall(Initialize)
if not ok then
    print("|cFFFF0000[Lore Archive] Critical UI Error:|r", err)
    error(err) -- Re-throw so BugGrabber picks it up
end
